...
function voltata_posted_on() {
	...
	if ( get_the_time( 'U' ) !== get_the_modified_time( 'U' ) ) {
		//$time_string = 'Updated on: <time class="entry-date published" datetime="%1$s">%2$s</time><time class="updated" datetime="%3$s">%4$s</time>';
	}
	...
	$posted_on = sprintf(
		esc_html_x( 'Posted on %s', 'post date', 'voltata' ),
...
function voltata_entry_footer() {
	...
	if ( 'post' === get_post_type() ) {
		...
		if ( $categories_list && voltata_categorized_blog() ) {
			printf( '<span class="glyphicon glyphicon-folder-open"></span>
			         <span class="cat-links">' . esc_html__( 'Posted in %1$s', 'voltata' ) . '</span>', $categories_list ); // WPCS: XSS OK.
		}

		/* translators: used between list items, there is a space after the comma */
		$tags_list = get_the_tag_list( '', esc_html__( ', ', 'voltata' ) );
		if ( $tags_list ) {
			printf( '<span class="glyphicon glyphicon-tags"></span>
			            <span class="tags-links">' . esc_html__( 'Tagged in %1$s', 'voltata' ) . '</span>', $tags_list ); // WPCS: XSS OK.
		}
	}

	if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
		echo '<span class="glyphicon glyphicon-comment"></span><span class="comments-link">';
		comments_popup_link( esc_html__( 'Leave a comment', 'voltata' ), esc_html__( '1 Comment', 'voltata' ), esc_html__( '% Comments', 'voltata' ) );
		echo '</span>';
	}

	edit_post_link( esc_html__( 'Edit', 'voltata' ), '<br/><br/><span class="edit-link">', '</span>' );
}
endif;
